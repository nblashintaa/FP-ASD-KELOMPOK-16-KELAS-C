...
public class Board {  // save as "Board.java"
    ...
    private boolean checkWin(Seed player, int row, int col) {
        return checkDirection(player, row, col, 0, 1) ||  // Horizontal
                checkDirection(player, row, col, 1, 0) ||  // Vertical
                checkDirection(player, row, col, 1, 1) ||  // Diagonal \
                checkDirection(player, row, col, 1, -1);   // Diagonal /
    }

    /** Check four in a row for the given direction */
    private boolean checkDirection(Seed player, int row, int col, int rowDir, int colDir) {
        ...
        return count >= 4; // Return true if at least 4 in a row
    }
    public void paint() {
        for (int row = 0; row < ROWS; ++row) {
            for (int col = 0; col < COLS; ++col) {
            ...
            if (row < ROWS - 1) {
                for (int i = 0; i < COLS; ++i) {
                    System.out.print("----");  // Adjusted separator for 8x8
